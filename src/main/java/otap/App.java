...
public class App
{
    ...
    public static void main( String[] arguments ) throws Exception
    {
        ...
        try (FileOutputStream fos = new FileOutputStream(filePath+appArgs.fileName);
            ...
            dos.writeInt(appArgs.versions.size()); // icapRomVersionMidUpdateCount
            for (int index=0; index<appArgs.versions.size(); index+=1) {
                dos.writeInt(appArgs.updateTypeIds.get(index));
                dos.writeInt(appArgs.versions.get(index));
            }
        } catch (Exception e) {
